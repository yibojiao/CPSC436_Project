class BubbleChart {
    ...
    renderVis() {
        ...
        iconsEnter
            ...
            .attr("r", (d) => vis.radiusScale(d.total_emissions))
            .attr("opacity", (d) => {
                return d.total_emissions >= 1.5 ? 1 : 0;
            })
            .attr("fill", (d) => vis.drawIcon(d.id))
            .on("mousemove", function (event, data) {
                d3.select(this).transition().duration(200).attr("opacity", 1);
                vis.config.calltooltip(event, data, vis);
            })
            ...
            .on("click", function (event) {
                ...
                if (!isActive) {
                    ...
                    const selectedFood = vis.svg.selectAll(".icon.active").data();
                    vis.dispatcher.call("reset", event);
                    vis.dispatcher.call("select", event, selectedFood[0]);
                } else {
                    vis.svg.selectAll(".icon.active").classed("active", false);
                    vis.svg.selectAll(".icon").attr("stroke", "none").attr("filter", "none");
                    vis.dispatcher.call("reset", event);
