class RingChart {
  ...
  initVis() {
    ...
    vis.g.append('circle').attr('cx', 150).attr('cy', 245).transition().duration(800).attr('r', 40)
        .attr('fill', 'url(#Icon)');
    vis.g.append('text').text('Food Groups')
      .attr('font-size', '30px')
      .attr('font-family', 'Marker Felt')
      .attr('transform', `translate(20, 35)`);
    vis.g.append('text').text('We devied all 43 foods into 4 groups:')
      .attr('fill', '#5a5a5a').attr('font-size', '15px')
      .attr('transform', `translate(20, 65)`);
    vis.g.append('text').text('proteins, fruits&veggies, grains, ')
      .attr('fill', '#5a5a5a').attr('font-size', '15px')
      .attr('transform', `translate(20, 85)`);
    vis.g.append('text').text('oils&others.')
      .attr('fill', '#5a5a5a').attr('font-size', '15px')
  ...
  updateVis() {
      ...
      vis.ringData = [
        {id: 'protein', name: 'protein', value: protein},
  ...
  renderVis() {
      let vis = this;
      vis.rings = vis.g.selectAll('.cateArea').data(vis.arcData)
        .join('path')
        .attr('class', 'cateArea')
        .attr('id', d => d.data.id)
      ...
      vis.g.selectAll('.cateLabel').data(vis.arcData)
        .join('text')
        ...
        .attr('transform', d => `translate(${vis.arc.centroid(d)[0]+150}, ${vis.arc.centroid(d)[1]+245})`)
        .attr('font-size', '30px');

      vis.g.selectAll('.cateName').data(vis.arcData)
        .join('text')
        .attr('class', 'cateName')
        .append('textPath')
        ...
        .attr('fill', '#5a5a5a')
        .attr("startOffset", "2%")
        .attr('transform', d => `translate(${1.5*vis.arc.centroid(d)[0]+150}, ${1.5*vis.arc.centroid(d)[1]+245})`)
        .attr('font-size', '15px');
      if (vis.selection) {
        d3.selectAll(`.cateArea#${vis.selection}`).attr('fill', 'white')
          .transition().duration(500)
          .attr('d', d3.arc().outerRadius(120).innerRadius(40));
      } else {
        vis.rings.attr('fill', '#999999')
          .attr('d', d3.arc().outerRadius(100).innerRadius(40));
      }
  }
}